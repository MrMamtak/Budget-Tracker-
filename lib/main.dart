...
class BudgetProvider extends ChangeNotifier {
  ...
  Future<void> loadData() async {
    ...
    final str = prefs.getString('budget_data');
    if (str != null) {
  ...
  Future<void> saveData() async {
    final prefs = await SharedPreferences.getInstance();
    final str = json.encode(projects.map((p) => p.toJson()).toList());
    await prefs.setString('budget_data', str);
  }
  ...
  void duplicateProject(String id) {
    final orig = projects.firstWhere((p) => p.id == id);
    final copy = Project(
...
class ProjectListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Projects'),
        actions: [
          IconButton(icon: Icon(Icons.refresh), onPressed: prov.resetAll),
        ],
      ),
      body: ListView(
        children: prov.projects
            .map((p) => ListTile(
                  title: Text(p.name),
                  subtitle: Text(
                      'Budget: \$${p.budget.toStringAsFixed(2)} | Spent: \$${p.tasks.fold(0, (s, t) => s + t.amount).toStringAsFixed(2)}'),
                  ...
                  trailing: PopupMenuButton<String>(
                    onSelected: (v) {
                      if (v == 'edit') _editProject(context, p);
                      if (v == 'delete') prov.deleteProject(p.id);
                      if (v == 'dup') prov.duplicateProject(p.id);
                    },
                    itemBuilder: (_) => [
  ...
  void _newProject(BuildContext ctx) {
        ...
        actions: [
          ...
          ElevatedButton(
              ...
              onPressed: () {
                ...
                ctx.read<BudgetProvider>().addProject(p);
                Navigator.pop(ctx);
              }),
        ],
      ),
  ...
  void _editProject(BuildContext ctx, Project p) {
    final nameCtrl = TextEditingController(text: p.name);
    final budgetCtrl = TextEditingController(text: p.budget.toString());
    showDialog(
      context: ctx,
      builder: (_) => AlertDialog(
          ...
          ElevatedButton(
              ...
              onPressed: () {
                final updated = Project(
                  id: p.id,
                  name: nameCtrl.text,
                  budget: double.tryParse(budgetCtrl.text) ?? p.budget,
                  tasks: p.tasks,
                );
                ctx.read<BudgetProvider>().updateProject(updated);
                Navigator.pop(ctx);
              }),
        ],
      ),
...
class ProjectDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final prov = context.watch<BudgetProvider>();
    final proj = prov.projects.firstWhere((p) => p.id == project.id);
    final spent = proj.tasks.fold(0.0, (s, t) => s + t.amount);
          ...
          SizedBox(
            ...
            child: charts.PieChart([
              charts.Series<Task, String>(
                ...
                data: proj.tasks,
                domainFn: (t, _) => t.desc,
                measureFn: (t, _) => t.amount,
                labelAccessorFn: (t, _) => t.desc,
              )
            ], animate: true, defaultRenderer:
          ...
          Expanded(
            child: ListView(
              children: proj.tasks
                  .map((t) => ListTile(
                        title: Text(t.desc),
                        subtitle: Text('\$${t.amount} | ${t.category} | ${t.date}'),
  ...
  void _newTask(BuildContext ctx, Project p) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: Text('New Task'),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: descCtrl, decoration: InputDecoration(labelText: 'Desc')),
          TextField(
              controller: amtCtrl,
        ...
        actions: [
          ...
          ElevatedButton(
              ...
              onPressed: () {
                ...
                p.tasks.add(task);
                ctx.read<BudgetProvider>().updateProject(p);
                Navigator.pop(ctx);
              }),
        ],
      ),
  ...
  void _editTask(BuildContext ctx, Project p, Task t) {
    ...
    final dateCtrl = TextEditingController(text: t.date);
    showDialog(
      context: ctx,
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
              child: Text('Save'),
              onPressed: () {
                t.desc = descCtrl.text;
                t.amount = double.tryParse(amtCtrl.text) ?? t.amount;
                t.category = catCtrl.text;
                t.date = dateCtrl.text;
                ctx.read<BudgetProvider>().updateProject(p);
                Navigator.pop(ctx);
              }),
        ],
